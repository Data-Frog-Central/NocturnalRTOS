...
static unsigned char *usbDataBufferIn;
...
static int usb_upgrade_process(void)
{
	...
	unsigned char __usbDataBufferIn[64];
	...
	usbDataBufferIn = __usbDataBufferIn;

	BUSB_Init(&musbObj[0]);
	BUSB_Init(&musbObj[1]);

	while (1) {
		BUSBIrqRead(&musbObj[0]);
		BUSBIrqRead(&musbObj[1]);
		...
		if (elapsed > pbootinfo->hcprogrammer_sync_detect_timeout) {
			return 0;
		}
	}
