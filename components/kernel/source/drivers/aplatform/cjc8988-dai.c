			      ...
			      uint8_t align)
{
	...
	if (snd_direction & SND_IN) {
		...
		res += cjc8988_Write_Reg(fd, 0x01, 0x1f);//L, 0dB
		res += cjc8988_Write_Reg(fd, 0x03, 0x1f);//R, 0dB
		...
		res += cjc8988_Write_Reg(fd, 0x2B, 0xa8);//-13dB
		res += cjc8988_Write_Reg(fd, 0x2D, 0xa8);//-13dB
	}
...
int reset_cjc8988(void)
{
	...
	ioctl(fd, SND_IOCTL_SET_MUTE, 1);//强制mute
	...
	ioctl(fd, SND_IOCTL_SET_MUTE, 0);//释放mute gpio的控制
	close(fd);
...
static int cvbs_audio_open(int argc, char *argv[])
{
	...
	printf("ioctl````\n");
	struct snd_cjc8988_input input = {"linein"};
	ioctl(i2si_fd, SND_IOCTL_SET_CJC8988_INPUT, &input);
