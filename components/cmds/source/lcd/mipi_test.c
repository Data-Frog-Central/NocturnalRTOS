...
static int mipi_sdi_get_timing_cb(int argc, char *argv[])
{
	...
	ioctl(fb,MIPI_DSI_GET_TIMING,&timing);
	printf("get timing -C %d -t %d -a %d -f %d -s %d -b %d -T %d -A %d -F %d -S %d -B %d -c %d -o %d -l %d -m %d -H %d -V %d\n",
...
static void mipi_sdi_set_timing_print_usage(const char *prog)
{
		...
		puts(	"  -C --clock			set mipi timing clock-frequency\n"
				...
				"eg: timing_set -C 0 -t 1228 -a 1080 -f 116 -s 16 -b 16 -T 2015 -A 1920 -F 59 -S 4 -B 32 -c 28 -o 14 -l 4 -m 5 -H 1 -V 1"
		);
}
...
static int mipi_sdi_set_timing_cb(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "C:t:a:f:s:b:T:A:F:S:B:c:o:l:m:H:V:h:", lopts, NULL);
		// printf("%s %d %c\n",__func__,__LINE__,c);
		if (c == -1) {
			break;
		}
	...
	printf("get timing -C %d -t %d -a %d -f %d -s %d -b %d -T %d -A %d -F %d -S %d -B %d -c %d -o %d -l %d -m %d -H %d -V %d\n",
										...
										timing.packet_cfg,timing.pclk,timing.lane_num,timing.color_mode,\
										timing.h_sync_level,timing.v_sync_level);
	ioctl(fb,MIPI_DSI_SET_TIMING,&timing);
	close(fb);
	return 0;
}
...
static int mipi_sdi_set_timing_init_cb(int argc, char *argv[])
{
	...
	ioctl(fb,MIPI_DSI_TIMING_INIT,NULL);
	close(fb);
}
...
CONSOLE_CMD(lvds_gpio, "mipi_test", lvds_gpio_test, CONSOLE_CMD_MODE_SELF, "lvds_gpio_test, eg: lvds_gpio 10 1")
CONSOLE_CMD(gpio, "mipi_test", gpio_set_out, CONSOLE_CMD_MODE_SELF, "gpio_set_out, eg: gpio 99 1")
CONSOLE_CMD(wr, "mipi_test", mipi_dsi_dcs_wr_cb, CONSOLE_CMD_MODE_SELF, "Mipi read / write eg, eg: wr")
